...
class AtomicworkMCPServer {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description: "List all requests/tickets in the workspace (not just assigned to you)",
            inputSchema: {
              type: "object",
              properties: {
                status: {
  ...
  private async handleListAllRequests(args: any) {
    try {
      ...
      const endpoint = `/workspaces/${WORKSPACE_ID}/requests/list?filter_name=all&sort_order=CREATED_AT_DESC&page=1&per_page=${limit}`;
